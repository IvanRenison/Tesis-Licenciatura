...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \pause % Capás tengo que escribir (además de decir) porqué es semi-decidible
    \item Hay algunos conjuntos $G$ para los cuales hay un algoritmo fácil. \pause \\ % Poner en otro frame
    ...
    \item % Agregar que si es finita se puede usar el algoritmo sensillo
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \pause
  \begin{itemize}
    \item Implementa estructuras para monomios y polinomios con sus operaciones básicas. % Poner que operaciones
    \pause
    \item Implementa Buchberger y F4.
    \pause
    \item Funciona para un \alert{cuerpo arbitrario}. % Es paramétrica en el cuerpo
    \pause
...
\begin{frame}[shrink]
  \frametitle{Comparación} % Mostrar unos pocos
  \setlength{\jot}{0pt}
  \begin{align*}
    \text{FK2} =& \{a^2\} \\
