...
\renewenvironment{proof}[1][\proofname]{{\bfseries #1: }} % Estilo del título de la demostración
...
\begin{definition}
Fijemos $X = \{x_1, …, x_n\}$ y un orden total entre $X$: $x_1 ≤ … ≤ x_n$, el cual se extiende (como es usual) de forma lexicográfica a $⟨X⟩$. El orden lexicográfico por grado $ ≤_{deglex}$ sobre $⟨X⟩$ se define así:
...
$∀i ∈ ℕ : \lm(P_i) = \lm(P_1)$
\end{fact}
En efecto, no puede ser $\lm(P_i) < \lm(P_1)$ porque entonces $P_i, P_{i + 1}, …$ sería una cadena estrictamente decreciente infinita que rompería la minimalidad de $\lm(P_1)$.
...
\begin{enumerate}[leftmargin=*,label=(\alph*),topsep=4pt,itemsep=2pt,partopsep=2pt, parsep=2pt] % Hacer global
...
Esto significa que $f →^*_{≤, G} 0$ y por ende por (2) $f ∈ I$
