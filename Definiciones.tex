...
Sea $i$ tal que $m_i = \lm_≤(agb)$, el cual existe por (2).

Notar también que $m_i = \lm_≤(\frac{f_{\lm_≤(agb)}}{\lc_≤(g)}agb)$ (4) porque es multiplicar por un escalar adentro del $\lm$ en la condición de $i$.

Si $i ≠ 0$:

  (4) significa que los términos $c_1 m_1, c_2 m_2, …, c_{i-1}, m_{i-1}$ son iguales en $f$ y en $f'$ y no hay nada mas en el medio, porque $f'$ es $f$ con cosas menores o iguales a $\lm_≤(\frac{f_{\lm_≤(agb)}}{\lc_≤(g)}agb)$ restadas (por (3)).
...
Si $i = 0$ aplica lo mismo pero directamente a $m_0$ solo que sin tener en cuenta los $m$ anteriores.
...
&f_1 + f →_{≤, F} f_1 + f - \frac{f_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \Arrow{(4)} \\
& ⇒ f_1 + f →_{≤, F} f_1 + f + \frac{{f_0}_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \Arrow{(2)} \\
& ⇒ f_1 + f →_{≤, F} f_0 + f \\
& ⇒ f_1 + f ↓_{≤, F} f_0 + f
\end{DispWithArrows*}
...
& ⇒ f_0 + f →_{≤, F} f_0 + f - \frac{{f_0}_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb - \frac{f_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \Arrow{(2)}\\
...
Los casos (2) $⇒$ (5) y (5) $⇒$ (4) son muy fáciles y quedan como ejercicio. % Pongo esto hasta que la haga
