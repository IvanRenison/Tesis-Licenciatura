...
Se puede probar que si $α = (a, b, c, d, f, g)$ es una ambigüedad entonces $\lm_≤{(afb)} = \lm_≤{(cgd)}$
...
Se puede probar que $\lm_≤{(\S(α))} < \lm_≤{(α)}$
...
Si $G$ es finito se puede probar que $\B_{e_≤}(G)$ es una base de Gröbner reducida de $(G)$, y además, que si $(G)$ tiene una base finita entonces $∃i ∈ ℕ : \B_{e_≤}^i(G) = (G)$ y si $(G)$ no tiene una base finita, entonces los $\B_{e_≤}^i(G)$ van enumerando a $(G)$
