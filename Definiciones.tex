...
\begin{definition}
Sea $≤$ un orden total sobre $⟨X⟩$: % TODO: Preguntar como hacer esto
...
\[ ≤_{deglex} ⊆ ⟨X⟩^2 \] % Preguntar como harían esto
...
\[ I\text{ es un ideal de }R\text{ si y solo si:} \]
\begin{enumerate}[label = (\alph*)]
\item $I ≠ Ø$
...
\[ ≡_I ⊆ R^2 \] % Preguntar mismo que mas arriba
\[ a ≡_I b ⇔ a - b ∈ I \]
\end{definition}
...
Ahora volvemos al álgebra libre.
...
\[f < g\text{ si y solo si vale alguna de las siguientes:}\]
\begin{enumerate}[label = (\alph*)]
\item $f = 0 ∧ g ≠ 0$
\item $\lm_≤(f) < \lm_≤(g)$
...
\end{fact}
\begin{proof}
Esto por aplicar la definición del orden polinomial, por la \cref{fact:≤ en KX no cadenas dec inf:1} y por lo que estamos suponiendo sobre $P$.
\end{proof}
...
La \cref{fact:→ achican:3} significa que los términos $c_1 m_1, c_2 m_2, …, c_{i-1}, m_{i-1}$ son iguales en $f$ y en $f'$ y no hay nada mas en el medio, porque $f'$ es $f$ con cosas menores o iguales a $\lm_≤(\frac{f_{\lm_≤(agb)}}{\lc_≤(g)}agb)$ restadas (por (2)).
...
Como además $\lm_≤(agb) ∉ \sop(f_1)$ por el antecedente, tenemos:
...
\item[Prueba de $↔^*_{≤, F} ⊆ ≡_{(F)}$]\

Como tanto $↔^*_{≤, F}$ como $≡_{(F)}$ son relaciones de equivalencia y además $↔^*_{≤, F}$ es la mínima relación de equivalencia que contiene a $→_{≤, F}$, alcanza con probar $→_{≤, F} ⊆ ≡_{(F)}$

Supongamos $f →_{≤, F} f'$ y probemos $f ≡_{(F)} f'$

Sean $g ∈ F, a, b ∈ ⟨X⟩$ tales que $f' = f - \frac{f_{\lm_≤(agb)}}{\lc_≤(g)}agb$, los cuales existen por lo que estamos suponiendo y por la definición de $→_{≤, F}$
...
Vamos a probar (1) $⇒$ (2), (2) $⇒$ (1), (2) $⇔$ (3), (2) $⇒$ (4), (4) $⇒$ (3), (2) $⇒$ (5) y (5) $⇒$ (4)
\begin{description}
...
Supongamos $∀f ∈ K⟨X⟩ : (f ∈ I ⇔ f →^*_{≤, G} 0)$ (6), tenemos que probar que $G$ es una base de Gröbner de $I$, o lo que es equivalente por definición de base de Gröbner:
\[(G) = I ∧ →_{≤, G}\text{ es confluente}\]
...
\item[Prueba de $→_{≤, G}$ es confluente]\
...
\item[(2) $⇒$ (4)] lo probamos suponiendo (2)
...
\begin{fact}\label{fact:Buchberger correctitud:3}
$∀i ∈ ℕ : \B_{e_≤}^{i}(F) ⊆ (F)$
\end{fact}
\begin{proof}
...
\begin{fact}\label{fact:Buchberger correctitud:4}
$∀i ∈ ℕ : \B_{e_≤}(F) ⊆ (F)$
\end{fact}
\begin{proof}
Esto es consecuencia directa de la \cref{fact:Buchberger correctitud:3} y la definición de $\B_{e_≤}$
\end{proof}

\begin{fact}\label{fact:Buchberger correctitud:5}
$(\B_{e_≤}(F)) = (F)$
\end{fact}
\begin{proof}
Esto vale porque por el caso base de la definición de $\B_{e_≤}^i$ tenemos $F ⊆ \B_{e_≤}$ y por la \cref{fact:Buchberger correctitud:4}.
\end{proof}

\begin{fact}\label{fact:Buchberger correctitud:6}
...
Y ahora, por \cref{fact:Buchberger correctitud:5} y \cref{fact:Buchberger correctitud:6} vale la definición de base de Gröbner, así que queda probado (1).
...
\begin{DispWithArrows*}
&f' = ∑_{i = 1}^{n'} a'_i g_{i'_i} b'_i \Arrow{Definición de $g_{i_i}$} \\
