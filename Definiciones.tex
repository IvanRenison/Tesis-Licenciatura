...
\begin{theorem}\label{thm:confluente_sii_Church-Rosser}
...
\begin{theorem}[$≤$ no tiene cadenas estrictamente decrecientes infinitas]\label{thm:le_no_cadenas_dec_inf}\
...
\begin{definition}[Congruencia modulo un ideal]\label{def:congruencia_mod_ideal}
...
\begin{theorem}\label{thm:congruencia_mod_ideal_es_equivalencia}
Sean $R$ un anillo y $I ⊆ R$ un ideal:

  $≡_I$ es una relación de equivalencia
\end{theorem}

\begin{theorem}\label{thm:en_ideal_sii_congruente_cero}
...
(tomar este mínimo está bien por el teorema \ref{thm:le_no_cadenas_dec_inf}) % Mencionar el teorema sobre monomios
...
\begin{theorem}[Clausura reflexo transitiva de las reducciones]\label{thm:CRT_reduciones}
Sean $G ⊆ K⟨X⟩$, entonces:

  $↔^*_{≤, G} = ≡_{(G)}$

(Donde $≡_{(G)}$ la congruencia modulo un ideal definida en la definición \ref{def:congruencia_mod_ideal})
\end{theorem}
\begin{proof}
...
\begin{algorithm}[H] % La H es para que se quede acá, porque se iba a otra página. Estaría bueno hacerlo global
  \caption{Ejemplo de estrategia de reducción}\label{alg:estrategia_de_reduccion}
...
  $⇔$ (Teorema \ref{thm:en_ideal_sii_congruente_cero})

    $f ≡_I 0$

  $⇔$ (Teorema \ref{thm:CRT_reduciones})

    $f ↔^*_{≤, G} 0$

  $⇔$ (Al ser $G$ una base de Gröbner $→_{≤, G}$ es confluente y por ende por el teorema \ref{thm:confluente_sii_Church-Rosser} es Church-Rosser, así que aplico el sii de Church-Rosser)
...
  $⇔$ (Como $0$ es el mínimo elemento y el teorema \ref{thm:reducciones_achican} dice que las reducciones achican el segundo término del $∧$ ocurre solo para $f' = 0$)
