...
La respuesta a esa pregunta es que no, porque el problema es indecible. Sin embargo, si existen algoritmos que cuando $f ∈ (G)$ termina y devuelve una prueba, pero que cuando $f ∉ (G)$ puede no terminar. Sobre esos algoritmos se trata este trabajo.
...
(Donde $≡_{(G)}$ es la congruencia modulo un ideal definida en la definición \ref{def:congruencia mod ideal})
\end{theorem}
\begin{proof}
