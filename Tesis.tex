...
\begin{definition}\label{def:cosas de polinomios}
  ...
  \begin{itemize}
    \item el coeficiente de $m$ en $f$ es $f_m = \begin{cases} c_0&\text{si }m = m_0 \\ ⋮ & \\ c_n&\text{si }m = m_n \\ 0&\text{si no} \end{cases} $. % CONSULTAR
...
\begin{proof} Probemos las dos inclusiones.
  \begin{description}
    ...
    \begin{DispWithArrows*}
      ...
      & ⇒ ∀i ∈ \{1, …, n\} : f_{i - 1} ↓_{≤, G} f_i \\ % Debería agregar un lema para este paso? % CONSULTAR
...
\noindent \begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture} % Esto se veo feo % CONSULTAR
