...
\begin{definition}\label{def:estrategia de reducción}
...
\begin{theorem}[Condición de Buchberger]\label{thm:condición de Buchberger}
  Sean $I$ un ideal de $K⟨X⟩$ y $G ⊆ K⟨X⟩$. Son equivalentes:
  \begin{enumerate}
    \item $G$ es una base de Gröbner de $I$.
...
\end{lemma}
\begin{proof}
  Procedemos por inducción sobre $i$. El caso base se cumple porque $G ⊆ (G)$. Para el caso inductivo, supongamos que la afirmación es cierta para $i$ y probemos que también vale para $i + 1$:
  ...
  Dividimos en casos según ese $∨$:

  \begin{description}
    \item[Caso $f ∈ \B_{e_≤}^i(G)$:] Es valido por ser exactamente la hipótesis inductiva.
    \item[Caso $∃α ∈ \amb(\B_{e_≤}^i(G)) : f = e_≤(\B_{e_≤}^i(G))(\S(α))$:] Tomemos $α$ que satisface el $∃$.
    Por el \cref{lemma:e mantiene pertenencia a ideal} vale que $f ∈ \B_{e_≤}^i(G)$ y por la hipótesis inductiva que $f ∈ (G)$.
  \end{description}
...
\end{lemma}
\begin{proof}
  Tomemos $α = (a, b, c, d, f, g) ∈ \amb(\B_{e_≤}(G))$. Por definición de $\amb$, se cumple que $f, g ∈ \B_{e_≤}(G)$. Sea $i ∈ ℕ$ el mínimo tal que $f ∈ \B_{e_≤}^i(G)$, $j ∈ ℕ$ el mínimo tal que $g ∈ \B_{e_≤}^j(G)$ y $k = \max(i, j)$.
  ...
  Combinando eso con que por definición de estrategia de reducción vale que $\S(α) →^*_{≤, \B_{e_≤}^k(G)} e_≤(\B_{e_≤}^k(G))(\S(α))$ y por ende $\S(α) →^*_{≤, \B_{e_≤}(G)} e_≤(\B_{e_≤}^k(G))(\S(α))$ vale que $\S(α) →^*_{≤, \B_{e_≤}(G)} 0$.
\end{proof}

\begin{proof}[Demostración del \cref{thm:Buchberger correctitud}]
  Por el \cref{lemma:Buchberger correctitud:5} y el \cref{lemma:Buchberger correctitud:6} vale la equivalencia de base de Gröbner de la condición de Buchberger (\cref{thm:condición de Buchberger}), así que (1) queda probado.

  Para probar (2), asumamos el antecedente, o sea, que $(G)$ tiene una base de Gröbner finita, y tomemos una base finita $\{g_1, …, g_n\}$. Ahora tenemos que probar el consecuente, o sea, que $∃i ∈ ℕ : (\B_{e_≤}^i(G))$ es una base de Gröbner.
...
Ese cambio es en lugar de para las reducciones usar el conjunto anterior usar también los nuevos polinomios que ya fueron agregados, es decir, llamar a $e_≤$ con $new\_B$ en lugar de con $B$ en la línea 6. % Se podrá hacer con un label esto?
...
\begin{definition}
Sean $M = \{m_1, …, m_n\} ⊆ ⟨X⟩$ con $m_1 > ⋯ > m_n$. $\mat_T$ aplicado a un polinomio se define como la inversa de $\poli_T$:
