...
Sobre estas propiedades los siguientes teoremas y lemas:
...
\begin{definition}
  ...
  La suma en $R⟨X⟩$ se define de la manera esperable. El producto por escalares de define como:
...
Para definirla, además de definirse para un conjunto, se define de forma auxiliar para: un solo polinomio y para un polinomio y dos monomios.

\begin{definition}
  Sean $G ⊆ K⟨X⟩$, $g ∈ K⟨X⟩ - \{0\}$, $a, b ∈ ⟨X⟩$ y $f, f' ∈ K⟨X⟩$. Se definen las relaciones $→_{≤, a, g, b}$, $→_{≤, g}$ y $→_{≤, G}$ del siguiente modo:
  \begin{itemize}
    \item $f →_{≤, a, g, b} f' ⇔ \lm_≤(agb) ∈ \sop(f) ∧ f' = f - \frac{f_{\lm_≤(agb)}}{\lc_≤(g)}agb$.
    \item $→_{≤, g} = ⋃_{a, b ∈ ⟨X⟩} →_{≤, a, g, b}$.
    \item $→_{≤, G} = ⋃_{g ∈ G} →_{≤, g}$.
  \end{itemize}
  Y cuando vale $f →_{≤, G}f'$ se dice que $f$ se reduce a $f'$ con $G$.
\end{definition}

Como teníamos la \cref{def:operaciones relaciones} y la \cref{def:forma normal} quedan definidas automáticamente las relaciones $→^*_{≤, …}$, $←_{≤, …}$, $↓_{≤, …}$, etc.
...
\begin{lemma}\label{lemma:suma →↓}
  Sean $G ⊆ K⟨X⟩$ y $f, f_0, f_1 ∈ K⟨X⟩$, entonces:
  \[ f_0 →_{≤, G} f_1 ⇒ f_0 + f ↓_{≤, G} f_1 + f \text{.}\]
\end{lemma}
\begin{proof}
  Supongamos el antecedente $f_0 →_{≤, G} f_1$.

  Sean $g ∈ G, a, b ∈ K⟨X⟩$ tales que $\lm(agb) ∈ f_0$ y $f_1 = f_0 - \frac{{f_0}_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb$, los cuales existen por aplicar la definición de $→_{≤, G}$ en lo que estamos suponiendo.

  Dividamos en casos según la pertenencia $\lm_≤(agb)$ a $\sop(f)$:
  ...
  \begin{description}
    ...
    \begin{description}
      ...
      Por (i) e (ii) tenemos $f_0 + f ↓_{≤, G} f_1 + f$.
    \end{description}
  \end{description}
\end{proof}
...
\begin{proof}
  ...
  \begin{description}
    ...
    Sean $g ∈ G, a, b ∈ K⟨X⟩$ tales que $\lm(agb) ∈ f$ y $f' = f - \frac{f_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb$, los cuales existen por aplicar la definición de $→_{≤, G}$ en lo que estamos suponiendo. Tenemos:
...
\end{theorem}
\begin{proof}
  Lo vamos a demostrar por contradicción. Supongamos que $→_{≤, G}$ no es terminante. Por definición de terminante, podemos tomar una sucesión $P ∈ K⟨X⟩^ℕ$ tal que:
  ...
  Pero esto contradice el \cref{lemma:≤ en KX no sucesiones dec inf} que dice que no hay sucesiones estrictamente decrecientes infinitas en $K⟨X⟩$.
\end{proof}
...
\begin{definition}\label{def:estrategia de reducción}
  Sea $e_≤ : 𝒫(K⟨X⟩) → K⟨X⟩ → K⟨X⟩$, se define:
  \begin{itemize}
...
\begin{definition}
  ...
  Además, si $f, g ∈ K⟨X⟩$ se se define que $(a, b, c, d, f, g)$ es una ambigüedad si y solo si $(a, b, c, d, \lm_≤{(f)}, \lm_≤{(g)})$ es una ambigüedad y lo mismo para ambigüedades de superposición, de inclusión y relevantes.
...
\begin{definition}
  Sean $a, b, c, d ∈ ⟨X⟩, f, g ∈ K⟨X⟩$ y $α = (a, b, c, d, f, g)$ una ambigüedad, se define el S-polinomio de $α$:
...
\begin{definition}
  Sean $G ⊆ K⟨X⟩$ y $e_≤$ una estrategia de reducción, se definen las bases de Buchberger como:
  \begin{itemize}
    \item $\B_{e_≤}^0(G) = G$.
