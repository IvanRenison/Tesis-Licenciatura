...
\begin{proof} Lo probamos probando que $f$ esta en uno si y solo si está en el otro.
  \begin{description}
    ...
    \item[Vuelta ($⇐$):] La vuelta es cierto porque como $c_i m_i, m'_i ∈ K⟨X⟩$ es un caso particular de la definición.
  \end{description}
\end{proof}
...
Por ejemplo, si tenemos $f = 2 cba + 3 abcda$, $g_0 = a - bcd$ y $G = \{g_0\}$ tenemos:

\[f →_{≤, G} f + 3a g_0 a = 3 aaa + 2 cba \text{.}\]
...
\begin{lemma}\label{lemma:suma →↓}
  Sean $G ⊆ K⟨X⟩$ y $f, f', f'' ∈ K⟨X⟩$. Entonces
  \[ f →_{≤, G} f' ⇒ f + f'' ↓_{≤, G} f' + f'' \text{.}\]
\end{lemma}
\begin{proof}
  Supongamos el antecedente $f →_{≤, G} f'$.

  Sean $g ∈ G, a, b ∈ K⟨X⟩$ tales que $f' = f - \frac{{f}_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb$, los cuales existen por definición de $→_{≤, G}$.
  ...
  \begin{description}
    ...
    \begin{DispWithArrows*}
      &f' = f - \frac{{f}_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} \\
      & ⇒ f' + f'' = f - \frac{{f}_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} + f'' \Arrow{Definición de $→_{≤, a, g, b}$}\\
      & ⇒ f + f'' →_{≤, a, g, b} f' + f'' \\
      & ⇒ f + f'' →_{≤, G} f' + f'' \Arrow{Definición de $↓$, ambos se reducen a $f' + f''$}\\
      & ⇒ f + f'' ↓_{≤, G} f' + f'' \text{.}
    \end{DispWithArrows*}

    \item[Caso $\lm_≤(agb) ∈ \sop(f'')$:]\
    \begin{description}
      \item[Subcaso $f_{\lm_≤(agb)} = -{f}_{\lm_≤(agb)}$]

      Este es el caso en el que $\lm_≤(agb)$ se cancela en la suma $f + f''$. Y como además $\lm_≤(agb) ∉ \sop(f')$ por el antecedente, tenemos:

      \begin{DispWithArrows*}
        &f' + f'' →_{≤, G} f' + f'' - \frac{f''_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \Arrow{Subcaso} \\
        & ⇒ f' + f'' →_{≤, G} f' + f'' + \frac{{f}_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \Arrow{Condición de $g$, $a$ y $b$} \\
        & ⇒ f' + f'' →_{≤, G} f + f'' \\
        & ⇒ f' + f'' ↓_{≤, G} f + f'' \text{.}
      \end{DispWithArrows*}

      \item[Subcaso $f''_{\lm_≤(agb)} ≠ -{f}_{\lm_≤(agb)}$:] En este caso $\lm_≤(agb)$ no se cancela en la suma $f + f''$, así que podemos aplicar $→_{≤, G}$:
      \begin{DispWithArrows*}
        &f + f'' →_{≤, G} f + f'' - \frac{(f + f'')_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \\
        & ⇒ f + f'' →_{≤, G} f + f'' - \frac{{f}_{\lm_≤(agb)} + f''_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \\
        & ⇒ f + f'' →_{≤, G} f + f'' - \frac{{f}_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb - \frac{f''_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \Arrow{Condición de $g$, $a$ y $b$}\\
        & ⇒ f + f'' →_{≤, G} f' + f'' - \frac{f''_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb  \text{.}
      \end{DispWithArrows*}
        Llamemos (i) a este último resultado.

      Además por el caso y el hecho de que $lm_≤(agb) ∉ \sop(f')$ también tenemos $\lm_≤(agb) ∈ \sop(f' + f'')$, entonces:
      \begin{DispWithArrows*}
        &f' + f'' →_{≤, G} f' + f'' - \frac{(f' + f'')_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \\
        & ⇒ f' + f'' →_{≤, G} f' + f'' - \frac{{f'}_{\lm_≤(agb)} + f''_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \Arrow{$\lm_≤(agb) ∉ \sop(f')$ porque $f →_{≤, G} f'$ y definición de $→_{≤, G}$}\\
        & ⇒ f' + f'' →_{≤, G} f' + f'' - \frac{0 + f''_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \\
        & ⇒ f' + f'' →_{≤, G} f' + f'' - \frac{f''_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb \text{.}
      \end{DispWithArrows*}
        Llamemos (ii) a este último resultado.

      Por (i) e (ii) tenemos $f + f'' ↓_{≤, G} f' + f''$.
    \end{description}
  \end{description}
\end{proof}
...
Probamos que $→_{≤, G}$ es terminante, pero no que sea confluente, ya que no lo es. Por ejemplo, siguiendo con el mismo ejemplo de antes en el que ya teníamos $f = 2 cba + 3 abcda$, $g_0 = a - bcd$, si ahora agregamos $g_1 = b - cdb$ y $G = \{g_0, g_1\}$ tenemos, como antes:

\[f →_{≤, G} f + 3a g_0 a = 3 aaa + 2 cba \text{.}\]

\noindent Y también:

\[f →_{≤, G} f - 3ab g_1 = 3 abb + 2 cba = f'' \text{.}\]
...
\begin{definition}
  ...
  Además, si $f, g ∈ K⟨X⟩$ se define que $(a, b, c, d, f, g)$ es una ambigüedad si y solo si $(a, b, c, d, \lm_≤{(f)}, \lm_≤{(g)})$ es una ambigüedad y lo mismo para ambigüedades de superposición, de inclusión y relevantes.
...
\begin{definition}
  Sean $a, b, c, d ∈ ⟨X⟩, f, g ∈ K⟨X⟩$ y $α = (a, b, c, d, f, g)$ una ambigüedad. Se define el S-polinomio de $α$ como
