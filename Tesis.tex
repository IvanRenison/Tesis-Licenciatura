...
Alguien podría preguntar que para qué sirve tratar de resolver ese problema y calcular bases de Gröbner. Mucha aplicación práctica no tiene. En el caso conmutativo si hay aplicaciones prácticas importantes por la relación que tiene con la geometría, pero en el no conmutativo no. Pero si hay aplicaciones en matemática teórica, que son de interés para Cristian Vay, el director de este trabajo. En particular en el calculo de dimensiones de algunas álgebras.
...
\begin{proof}
  ...
  \noindent Por la definición recursiva de $\B_{e_≤}^{i + 1}$ vale que $f ∈ \B_{e_≤}^i(G) ∨ ∃α ∈ \amb(\B_{e_≤}^i(G)) : f = e_≤(\B_{e_≤}^i(G))(\S(α))$. Dividamos en casos según ese $∨$
