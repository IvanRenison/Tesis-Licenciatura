...
En los primeros capítulos se proporciona una explicación matemática completa del tema, para que la tesis sea autocontenida y en los restantes se explica la librería de \cpp donde se implementó el cálculo (además de otras cosas necesarias) y se la compara con una de las implementaciones anteriores.
...
Para trabajar en algoritmos que aborden el \cref{problem:principal} se considera el conjunto de todos los polinomios no conmutativos que satisfacen ese $∃$, denotado como $(G)$, y se calcula lo que se llama una base de Gröbner de $(G)$. Una base de Gröbner de $(G)$ es un conjunto $G' ⊆ K⟨X⟩$ tal que $(G) = (G')$ y con el cual es posible, y fácil, computar si vale que $f ∈ (G')$. La base de Gröbner puede ser finita o infinita computacionalmente enumerable y el hecho de que a veces pase esto último es lo que hace que el problema no sea computable. % Pongo computacionalmente enumerable y no solo enumerable porque enumerable no dice nada

Para calcular esas bases de Gröbner hay dos algoritmos destacados, llamados el algoritmo de Buchberger y el algoritmo F4. La diferencia entre los dos algoritmos es que Buchberger va calculando la base de Gröbner de a un elemento por vez mientras que F4 usa herramientas de álgebra lineal para calcular de a muchos elementos al mismo tiempo.
