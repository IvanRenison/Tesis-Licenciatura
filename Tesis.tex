...
Un problema de decisión que existe sobre los polinomios no conmutativos es: dado un conjunto $G$ de polinomios no conmutativos y un polinomio no conmutativo $f$, decidir si $f$ se puede escribir como combinación lineal de elementos de $G$ con coeficientes en $K⟨X⟩$, o escrito formalmente, determinar si vale la siguiente expresión:
...
pero sí es semi-decidible. Esto significa que no hay un algoritmo que dice si vale la expresión o no, pero sí hay un algoritmo que en caso de que sí valga termina diciendo que sí y en caso de que no vale puede no terminar.
...
Para calcular esas bases de Gröbner hay dos algoritmos destacados, llamados el algoritmo de Buchberger y el algoritmo F4. Estos algoritmos, al igual que mucha de la teoría, están inspirados en la teoría para resolver el problema análogo para los polinomios conmutativos. De hecho, muchos de los nombres de las cosas se usaron primero para el caso conmutativo y después se usó el mismo nombre para el caso no conmutativo. Así, por ``algoritmo de Buchberger'' en otros contextos se puede estar refiriendo al algoritmo para el caso conmutativo. En esta tesis siempre se va a estar hablando del caso no conmutativo. Para una explicación del caso conmutativo ver \cite{book:ideals-varieties-algorithms}.
...
Para toda esta sección fijemos un conjunto $A$. Primero definimos algunas operaciones muy comunes sobre relaciones.
...
Como vamos a hablar de usar $→$ para transformar un objeto en otro, es útil la proxima definición que nos permite hablar de la forma normal de un objeto como el elemento al que se llega aplicando $→$ hasta que no se puede más.
...
El álgebra libre es básicamente el conjunto de los polinomios no conmutativos con sus operaciones.

En los polinomios no conmutativos los monomios son palabras y la única operación interna de los monomios (entre monomios y que devuelve monomios) es la multiplicación, que equivale a la concatenación de palabras.
...
Mas adelante va a ser necesario tener un orden entre los elementos de $⟨X⟩$, pero no es necesario fijar uno concreto, porque con que satisfaga la \cref{def:buen orden monomial} alcanza.
...
O sea, el orden lexicográfico por grado ordena primero por cardinalidad, que se llama grado también, y desempata con el orden lexicográfico. Por ejemplo, tenemos que $bc ≤_{deglex} abb$, $aabbc ≤_{deglex} abbcc$ y $ε ≤_{deglex} a$.

Se puede probar fácilmente que este orden es un buen orden monomial.
...
Los ideales son básicamente conjuntos cerrados por suma y producto por elementos cualquiera, y $(G)$ es el ideal generado por $G$.
...
\begin{definition}[Congruencia módulo un ideal]\label{def:congruencia mod ideal}
...
Esta congruencia es parecida a la de los números módulo otro número (de hecho, la congruencia de los números módulo otro número es un subcaso de esta tomando como ideal a todos los múltiplos del módulo) y vale que la clase de equivalencia del $0$ es el propio ideal:
...
En este orden, al igual que para los monomios, vale que no hay cadenas estrictamente decrecientes infinitas.

\begin{lemma}\label{lemma:≤ en KX no cadenas dec inf}
  La relación $≤$ en $K⟨X⟩$ no tiene cadenas estrictamente decrecientes infinitas.
\end{lemma}
\begin{proof}
  Supongamos que existen cadenas estrictamente decreciente infinita. Tomemos una $P$ que minimice $\lm(P_1)$. Tomar este mínimo es posible por el \cref{thm:≤ no cadenas dec inf}.
...
Ahora vamos a definir una relación de reducción en los polinomios no conmutativos (y acá es donde entra lo de sistemas de re-escritura).
...
Para definirla, además de definirla para un conjunto, la definimos para un solo polinomio y un polinomio y dos monomios de forma auxiliar.
...
Esta relación tiene un montón de propiedades que van a hacer falta y que se prueban a continuación.
...
\begin{lemma}[Clausura reflexo transitiva de las reducciones]\label{lemma:→^* = ≡}
  ...
  (Donde $≡_{(G)}$ es la congruencia módulo un ideal definida en la \cref{def:congruencia mod ideal})
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
  Si asumimos $f →^*_{≤, G} f'$, tenemos por el \cref{lemma:→^* = ≡} que $f ≡_{(G)} f'$ y entonces por el \cref{lemma:en ideal ⇔ congruente 0} tenemos $f ∈ (G) ⇔ f' ∈ (G)$.
\end{proof}
...
\begin{proof}
  ...
  Pero esto contradice el \cref{lemma:≤ en KX no cadenas dec inf} que dice que no hay cadenas estrictamente decrecientes infinitas en $K⟨X⟩$.
\end{proof}

\begin{theorem}[Caracterización de las formas normales de $→$]
  Sean $G ⊆ K⟨X⟩, f ∈ K⟨X⟩$, entonces: % Por algún motivo acá se pone solo un salto de linea extra
