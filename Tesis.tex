...
\begin{itemize}
  ...
  \item Tiffany Fuentes leyó la tesis e hizo algunas sugerencias.
...
Acá los $f_i$ y $f'_i$ se llaman cofactores. Por ejemplo con $K = ℚ$ y $X = \{x, y, z\}$, si:
\begin{align*}
  g_0 &= xy + yz \text{,} \\
  g_1 &= yx + zx \text{,} \\
  G &= \{g_0, g_1\} \text{,} \\
...
Si la explicación sobre el problema y los algoritmos no se entendió ahora no importa porque en el \cref{cap:Preliminares} se explica bien.
...
Sistemas de reescritura trata básicamente sobre el estudio de relaciones entre objetos con la idea de usar las relaciones para convertir un objeto en otro. En general usaremos relaciones denotadas con algún símbolo con forma de flecha porque se tiene la idea de que se \textsl{usa} la relación para convertir el objeto de la base de la flecha en el objeto de la punta de la flecha. Como recordatorio, una relación sobre $A$ es un subconjunto de $A^2$.
...
En el ejemplo de antes tenemos que $m_1 | m_0$ ya que $m_0 = x m_1 y$.
...
O sea, el orden lexicográfico por grado ordena primero por cardinalidad, también llamado grado, y desempata con el orden lexicográfico. Por ejemplo, tenemos que $yz ≤_{deglex} xyy$, $xxyyz ≤_{deglex} xyyzz$ y $ε ≤_{deglex} x$. Se puede probar fácilmente que este orden es un buen orden monomial.
...
\begin{definition}\label{def:cosas de polinomios}
  Sean $R$ un anillo conmutativo, $f ∈ R⟨X⟩$, $c_1, …, c_n ∈ R - \{0\}$ y $m_1, …, m_n, m ∈ ⟨X⟩$ con $f = ∑_{i = 1}^n c_i m_i$. Se definen:
  \begin{itemize}
...
Dicho en palabras, el orden en los polinomios es orden lexicográfico con el polinomio visto como una lista de monomios, sin coeficientes, ordenada de mayor a menor. Por ejemplo, tenemos estas desigualdades en $ℚ⟨x, y, z⟩$:
\begin{align*}
  x &< xy \text{,} \\
  yzz + z &< yzz + xyy \text{,} \\
  xz &< xz + xx \text{.}
...
\begin{proof}
  ...
  Como el \cref{lemma:0 es mínimo} dice que $0$ es un mínimo:
...
La relación $→_{≤, G}$ tiene varias otras propiedades útiles que se prueban a continuación.
...
\begin{theorem}\label{thm:equivalencias de base de Gröbner}
  ...
  \begin{enumerate}
    ...
    \item $(G) = I ∧ ∀f ∈ K⟨X⟩ : f ∈ I ⇔ f →^*_{≤, G} 0$. \label{thm:egb:2}
...
\begin{proof} Probaremos \ref{thm:egb:1} $⇒$ \ref{thm:egb:2}, \ref{thm:egb:2} $⇒$ \ref{thm:egb:3}, \ref{thm:egb:3} $⇒$ \ref{thm:egb:1}, \ref{thm:egb:4} $⇒$ \ref{thm:egb:3}, \ref{thm:egb:2} $⇒$ \ref{thm:egb:5} y \ref{thm:egb:5} $⇒$ \ref{thm:egb:4}.
  \begin{description}
    ...
    \item[\ref{thm:egb:1} $⇒$ \ref{thm:egb:2}:] Supongamos que $G$ es una base de Gröbner de $I$. Tenemos que probar $(G) = I ∧ ∀f ∈ K⟨X⟩ : f ∈ I ⇔ f →^*_{≤, G} 0$. El primer término del $∧$ es valido porque es parte de la definición de base de Gröbner. Para el segundo término tomemos $f ∈ K⟨X⟩$. Tenemos que probar $f ∈ I ⇔ f →^*_{≤, G} 0$. Vamos de un lado para el otro:
    \begin{DispWithArrows*}
    ...
    \begin{description}
      \item[Prueba de $(G) = I$:] Es valido porque es parte del antecedente.
...
Ahora probaremos que efectivamente los monomios principales se cancelan y otras propiedades y después enunciaremos el teorema para decidir si un conjunto es una base de Gröbner.
