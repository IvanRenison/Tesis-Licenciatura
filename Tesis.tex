...
\hypersetup{
  ...
  pdfkeywords = {
    ...
    Algoritmos de bases de Gröbner no conmutativas,
...
\begin{itemize}
  ...
  \item Algoritmos de bases de Gröbner no conmutativas
...
Este problema no es decidible porque el llamado problema de la palabra se reduce a este. En \cite{web:wiki:WordProblem} se puede ver qué es el problema de la palabra y en la Sección 1.3 de \cite{article:MORA1994131} cómo es la reducción. En esta tesis no se abordara el tema.

Para trabajar en algoritmos que aborden el \cref{problem:principal} se considera el conjunto de todos los polinomios no conmutativos que satisfacen ese $∃$, denotado como $(G)$, de forma que el \cref{problem:principal} se convierte en determinar si $f ∈ (G)$. Para responder a esto se usa el siguiente resultado: si $G = \{g_1, …, g_n\} ⊆ K⟨X⟩$, $f ∈ K⟨X⟩$, $a$ y $b$ son polinomios que consisten en una sola palabra y $c ∈ K$, entonces
\[ f ∈ (G) ⇔ f + c a g_i b ∈ (G) \text{.} \]

Teniendo en cuenta este resultado se intenta cancelar los términos de $f$ sumando distintos $c a g_i b$, y si en algún momento se llega a $0$ entonces vale que $f ∈ (G)$ porque el $0$ es la sumatoria vacía y porque todas las sumas mantiene la pertenencia a $(G)$.
