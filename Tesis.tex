...
\pagenumbering{gobble} % Eliminar los números de página, porque con margen inferior de un centímetro el número queda medio afuera, y además todos los lectores de PDF te dicen en que número de página estás
...
El capítulo incluye demostraciones de muchos de los resultados expuestos pero quienes tengan interés principalmente en la implementación de los algoritmos pueden obviar su lectura o leerlo solo por encima.
...
Mas adelante va a ser necesario tener un orden entre los elementos de $⟨X⟩$. Se podría fijar uno concreto definiendo directamente $≤$ para $⟨X⟩$, pero no hace falta, sino que se pueden enunciar las mínimas propiedades que harán falta y trabajar con cualquier otden que las satisfaga. Eso se logra con la siguiente definición.
...
\begin{definition}
  ...
  \noindent y el producto entre elementos de $R⟨X⟩$ definido como
...
\begin{definition}\label{def:cosas de polinomios}
  ...
  \begin{itemize}
    \item el coeficiente de $w$ en $f$ es $f_w = \begin{cases} c_0&\text{si }w = w_0 \\ ⋮ & \\ c_n&\text{si }w = w_n \\ 0&\text{si no} \end{cases} $. % CONSULTAR
    \item el soporte de $f$ es el conjunto $\sop(f) = \{w_1, …, w_n\}$.
...
Sobre estas definiciones valen muchas propiedades fáciles de probar, como por ejemplo $\lm(f) \lm(f') = \lm(f f')$, que durante el resto de la tesis vamos a usar mucho, pero no las vamos a probar una por una porque es tedioso y aburrido.
...
\section{Reducción de polinomios y bases de Gröbner} % Separar
...
\begin{definition}\label{def:reducciones}
  Sean $G ⊆ K⟨X⟩$ y $f, f' ∈ K⟨X⟩$. Se define la relacion $→_{≤, G}$ del siguiente modo:
...
Como teníamos la \cref{def:operaciones relaciones} y la \cref{def:forma normal} quedan definidas automáticamente las relaciones $→^*_{≤, G}$, $↔^*_{≤, G}$ y $↓_{≤, G}$. % Cristian me dijo de listar todas, pero lo que hice es listar las que se usan. ¿Está bien? % CONSULTAR
...
\begin{proof}
  ...
  \noindent Sea $i$ tal que $m_i = \lm_≤(agb)$, el cual existe por (i).

  \noindent Notar también que: % Cristian me dijo de poner punto seguidos. Lo que hice es poner \noindent. En otros lugares pasa lo mismo % CONSULTAR
...
\begin{proof}
  ...
  \noindent Sean $g ∈ G, a, b ∈ K⟨X⟩$ tales que $f_1 = f_0 - \frac{{f_0}_{\lm_≤(agb)}}{g_{\lm_≤(agb)}} agb$, los cuales existen por definición de $→_{≤, G}$.

  \noindent Dividamos la demostración en casos según la pertenencia de $\lm_≤(agb)$ a $\sop(f)$:
...
\end{theorem}
\begin{proof}
  Por contradicción, supongamos que tenemos $g ∈ G, m ∈ \sop(f)$ tales que $\lm(g) | m$.

  \noindent Sean $a, b ∈ ⟨X⟩$ tal que $m = agb$, los cuales existen por la definición de divisibilidad.
...
\begin{definition}\label{def:reductor}
  ...
  \[ e_≤\text{ es un reductor }⇔ ∀G ⊆ K⟨X⟩, f ∈ K⟨X⟩ : e_≤(G)(f)\text{ es forma normal de }f\text{ con respecto a }→_{≤, G} \text{.} \] % Cristian me dijo de poner "es una forma …", pero no lo puse porque la definición solo dice "es forma …" % CONSULTAR
\end{definition}
% Cristian dijo: En matemática pondríamos un símbolo de producto de conjuntos
% Pero se entiende, ¿no?
% Lo puse así porque tenía la idea de usar la aplicación parcial, pero creo que al final no la estoy usando (no estoy seguro, tendría que revisar)
% No se que será mejor hacer
% CONSULTAR
...
Antes definimos las ambigüedades, que representan los polinomios para los cuales puede pasar eso.
\begin{definition}
...
\begin{proof}\
  \begin{DispWithArrows*}
    &\lm_≤{(afb)} \Arrow{Definición de ambigüedad para polinomios} \\ % Cristian me dice de poner esto en la misma linea, pero qué hago con la flecha? % CONSULTAR
...
Lo de que los monomios principales se cancelen produce el siguiente lema. % No se bien como poner esto % CONSULTAR
...
La demostración se puede encontrar en \cite{phdthesis:Hof23}, en donde algo similar a esto está enunciado en el Teorema 2.4.54. No lo demostraremos acá porque requiere varios lemas adicionales que alargarían el trabajo.

Con esto ya se puede explicar la idea del algoritmo de Buchberger. Supongamos que tenemos un conjunto que no sabemos si es base de Gröbner o no y estamos chequeando si lo es con el \cref{thm:condición de Buchberger}. Si nos encontramos con un S-polinomio $f$ que se reduce a un polinomio $g$ distinto de $0$ lo que podemos hacer es agregar $g$ al conjunto y con eso $f$ sí se va a reducir a $0$. Se podría pensar que agregar $g$ por ahí cambia el ideal generado, pero por el \cref{lemma:S es cerrado en ideal} sabemos que $g$ es un elemento del ideal, por el \cref{thm:→ mantiene pertenencia a ideal} eso implica que $f$ también pertenece al ideal y por ende por el \cref{lemma:gen G = gen G U a con a ∈ gen G} se puede agregar al conjunto y que el ideal generado siga siendo el mismo. El problema con eso de agregar un nuevo polinomio es que si bien hay una S-polinomio que pasa a si reducirse a $0$ también aparecen nuevas ambigüedades. Eso sin embargo no es tanto problema porque lo se hace es hacer eso hasta que en algún momento todas las ambigüedades se reducen a $0$ y si eso no pasa nunca simplemente el algoritmo no termina nunca (lo que refleja la no decidibilidad del problema de pertenencia a un ideal). % Así está bien esto? No estoy seguro de que poner todo en un solo párrafo sea buena idea % CONSULTAR
...
\begin{definition}
  ...
  \end{itemize}
  A $\B_{e_≤}(G)$ se lo llama la base de Buchberger de $(G)$. % Qué tal esto así? % CONSULTAR
\end{definition}
...
Con lo que dijimos antes tiene sentido que (1) sea cierto, pero que (2) sea cierto no es para nada obvio. Para demostrar ambos items primero vamos a probar algunos lemas en el contexto de este teorema (o sea con $G$ y $e_≤$ fijados). % Como que estoy usando "demostrar" y "probar" mezclados, ¿qué debería hacer? $ CONSULTAR
...
\begin{proof}
  ...
  \noindent Por la definición recursiva de $\B_{e_≤}^{i + 1}$ vale que $f ∈ \B_{e_≤}^i(G) ∨ ∃α ∈ \amb(\B_{e_≤}^i(G)) : f = e_≤(\B_{e_≤}^i(G))(\S(α))$.

  Dividamos en casos según ese $∨$

  \begin{description}
    \item[Caso $f ∈ \B_{e_≤}^i(G)$:] Es valido por ser exactamente la hipótesis inductiva.
    \item[Caso $∃α ∈ \amb(\B_{e_≤}^i(G)) : f = e_≤(\B_{e_≤}^i(G))(\S(α))$:] En tal caso, por el \cref{lemma:e mantiene pertenencia a ideal} vale que $f ∈ \B_{e_≤}^i(G)(\S(α))$ y por la hipótesis inductiva que $f ∈ (G)$.
    % Cristian puso por la HI y Lema 2.47 tenemos que S(\alpha)\in(G) y entonces f\in(G) por Lema 2.40, pero me parece que no es cierto porque en todo caso daría que S(σ) ∈ (\B_{e_≤}^i(G)) % CONSULTAR
  \end{description}
...
\end{lemma}
\begin{proof}
  Tomemos $α = (a, b, c, d, f, g) ∈ \amb(\B_{e_≤}(G))$.

  \noindent Por definición de $\amb$ vale que $f, g ∈ \B_{e_≤}(G)$, así que sea $i ∈ ℕ$ el mínimo tal que $f ∈ \B_{e_≤}^i(G)$, $j ∈ ℕ$ el mínimo tal que $g ∈ \B_{e_≤}^j(G)$ y $k = \max(i, j)$.

  \noindent Por definición de $\amb$ tiene que valee que $α ∈ \amb(\B_{e_≤}^k(G))$ y entonces por definición de $\B_{e_≤}^i$ vale que $e_≤(\B_{e_≤}^k(G))(\S(α)) ∈ \B_{e_≤}^{k + 1}(G)$. Esto implica que $e_≤(\B_{e_≤}^k(G))(\S(α)) →_{≤, \B_{e_≤}^{k + 1}(G)} 0$ y por ende que $e_≤(\B_{e_≤}^k(G))(\S(α)) →_{≤, \B_{e_≤}(G)} 0$.

  \noindent Combinando eso con que por definición de reductor vale que $\S(α) →^*_{≤, \B_{e_≤}^k(G)} e_≤(\B_{e_≤}^k(G))(\S(α))$ y por ende $\S(α) →^*_{≤, \B_{e_≤}(G)} e_≤(\B_{e_≤}^k(G))(\S(α))$ vale que $\S(α) →^*_{≤, \B_{e_≤}(G)} 0$.
\end{proof}
...
El algoritmo así, si bien es una implementación tal cual de la definición de los conjuntos $\B_{e_≤}^i$ que probamos que es correcta, es muy lento y necesita un cambio para que pase a andar bien.
...
En la práctica el cambio ese hace una diferencia inmensa en la eficiencia, pasa de no andar rápido casi nunca a andar rápido en un montón de casos. El motivo teórico detrás de que hacer eso haga que pase a ser tanto mas rápido el autor no lo sabe ni lo pudo encontrar en la literatura.
...
% Cristian dijo: "mejorar la redacción". No hice casi ningún cambio, ¿está bien así? % CONSULTAR
...
La paralelización está hechoa con OpenMP (\cite{lib:openmp}), que es una libraría de C++ para hacer cosas en paralelo, así que para que corra en paralelo es igual que en cualquier otra cosa paralelizada con OpenMP.
...
Este benchmark imprime los tiempos promedios sobre todos los conjuntos generadores generados al azar y polinomios generados al azary también los tiempos máximos.
