...
\begin{theorem}\label{thm:equivalencias de base de Gröbner}
  ...
  \end{enumerate}
\end{theorem}
\begin{proof} Vamos a probar \ref{thm:egb:1} $⇒$ \ref{thm:egb:2}, \ref{thm:egb:2} $⇒$ \ref{thm:egb:3}, \ref{thm:egb:3} $⇒$ \ref{thm:egb:1}, \ref{thm:egb:3} $⇒$ \ref{thm:egb:2}, \ref{thm:egb:4} $⇒$ \ref{thm:egb:3}, \ref{thm:egb:2} $⇒$ \ref{thm:egb:5} y \ref{thm:egb:5} $⇒$ \ref{thm:egb:4}.
  \begin{description}
    ...
    Vamos a probar que el $∃$ de \ref{thm:egb:5}, se satisface de la forma $\lm(a_i g_i b_i) ≤ \lm(f)$ y $f = ∑_{i = 1}^n \frac{(f_{i-1})_{\lm_≤(a_i g_i b_i)}}{\lc_≤(g_i)} a_i g_i b_i$.
...
Es esperable que \ref{thm:Buchberger correctitud:item:1} sea cierto por como definimos la base de Buchberger, mientras que \ref{thm:Buchberger correctitud:item:2} es más sorprendente. Para demostrar ambos items primero vamos a probar algunos lemas en el contexto de este teorema (o sea con $G$ y $e_≤$ fijados).
...
\noindent \begin{tikzpicture}
  \begin{axis}[
      ...
      enlarge x limits = 0.03, % Espaciado en los bordes
      legend style = {at = {(0.024, 0.94)}, anchor = north west}, % Estilo de la leyenda
      legend entries = {Normal, Representación de cofactores}, % Entradas de la leyenda
  ]
    % Datos para cada conjunto
