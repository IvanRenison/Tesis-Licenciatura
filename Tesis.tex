...
Como ya se dijo antes, uno de los objetivos del trabajo fue paralelizar el cálculo de bases de Gröbner. Eso se hizo parcialmente para F4. O sea, se hizo, pero solo para una parte del código, así que no es demasiado útil. De cualquier manera, en esta sección se explica como hacer que el código corra en paralelo.

La paralelización está hecha con OpenMP (\cite{lib:openmp}), que es una libraría de C++ para de paralelización, así que para que corra en paralelo es igual que en cualquier otro código paralelizada con OpenMP.
...
Esa reducción concreta siempre trata de reducir por los primeros elementos del vector primero y trata de reducir primero los monomios más grandes del polinomio.
...
En este capítulo se presentan los resultados obtenidos tras corres los test que incluyen benchmarks.
...
Los tests que tienen benchmarks son todos salvo uno tests que usan los mains de la carpeta \texttt{mains} y mains hechos en Python con \texttt{operator\_gb} y son corridos con distintas entradas desde un archivo de Python. Los tiempos se miden desde el programa de Python, así que incluyen en tiempo hasta que se inicializa el programa y lee la entrada y el tiempo de impresión de la salida, pero como son siempre entradas y salidas relativamente chicas eso no debería ser problema.

Todos los tests que tienen benchmarks salvo uno funcionan tomando como input un conjunto generador que se sabe que el ideal generado tiene una base finita. Para estos benchmarks se usaron los siguientes conjuntos generadores:
...
\noindent Se sabe para $n ≤ 5$ los ideales generador por FK$n$ tienen bases de Gröbner finitas, pero $n > 5$ no se sabe. En \cite{web:Nichols} y \cite{book:introNichols} se puede encontrar mas información sobre las álgebras de Nichols.

Los tri que están porque \cite{thesis:Hof20} los usa, con ese mismo nombre, y el los sacó del Teorema 2.12 de \cite{book:class-tri}. Los trit son directamente sacados de la Proposición 1.9 de \cite{book:class-tri}.

En todos los benchmarks que usan estos casos los usan para los racionales, porque es para los racionales que se sabe que tienen bases finitas.
...
El test que usa los distintos algoritmos para calcular bases de Gröbner de conjuntos generadores que se sabe que sus ideales tienen bases de Gröbner finitas incluye mediciones de los tiempos y por supuesto usa los casos que vimos recién.
...
Se puede ver que aumentar los hilos hace que sea un poco más rápido, pero muy poco. Esto tiene sentido por lo de que solo la parte de agregar ambigüedades está paralelizada.
...
Se puede ver que la construcción de la respuesta lleva mas tiempo, pero en general no tanto mas. Acá no hay demasiado mas para ver mirando solo la parte de abajo, así que no se hace un gráfico con la parte de mas abajo.

\section{Pertenencia a ideales}

El test que para conjuntos generadores construidos al azar y polinomios construidos al azar ver con los distintos algoritmos si el polinomio está en el ideal generado o no, también mide los tiempos. Estos corren tanto para los racionales como para los enteros módulo un primo, pero para los raciones incluyen la comparación con \texttt{operator\_gb} y para los enteros módulo un primo no, porque \texttt{operator\_gb} es solo para racionales.

Este benchmark imprime los tiempos promedios sobre todos los conjuntos generadores construidos al azar y polinomios construidos al azar y también los tiempos máximos.

Estos son los resultados para los racionales (tal cual los imprime el test, \texttt{Buch} es Buchberger y \texttt{GB} es \texttt{operator\_gb}):
...
\begin{itemize}
  \item Es la primer implementación de bases de Gröbner no conmutativas en C++ (que el autor sepa). Esto es muy bueno porque C++ es un lenguaje particularmente rápido y en el que es fácil paralelizar. Esto es particularmente útil si se quieren correr casos que se demoran mucho.
  \item Es la primer implementación de bases de Gröbner no conmutativas que funciona para un cuerpo arbitrario (que el autor sepa). Esto está bueno para que si en algún momento alguien lo quiere usar para algún cuerpo distinto lo pueda hacer.
  \item La implementación de Buchberger incluye la parte de la construcción de la respuesta, que si bien es algo muy fácil, la única otra implementación que lo hace es \texttt{operator\_gb}.
\end{itemize}
...
\begin{itemize}
  ...
  \item Del algoritmo F4 (y de Buchberger) para el caso si conmutativo hay varias implementaciones en C++, por ejemplo \cite{lib:openf4, lib:mathic, lib:M4GB}, inclusive en paralelo, como \cite{DBLP:journals/jsc/Reeves98, lib:parallelGBC}. Muchas son solo para los enteros módulo un primo, pero hay algunas que también para los racionales. Es muy probable que analizando esas implementaciones se puedan sacar muchas cosas útiles, en particular, podría ser que ya alguien haya hecho la implementación de la reducción por filas de Faugère-Lachartre, ya que posiblemente sirva también para el caso no conmutativo. Así que eso sería una buena cosa para hacer.
  ...
  \item Algo que se podría hacer con la librería es analizar cuanto tiempo se consume en las distintas partes de los algoritmos, porque eso podría llegar a llevar a optimizaciones.
  ...
  Con los ideales a izquierda sobre polinomios no conmutativos también hay toda una teoría de bases de Gröbner. En \cite{phdthesis:Hof23} se puede encontrar una explicación sobre esta teoría.

  Implementar el cálculo de bases de Gröbner de la teoría de ideales a izquierda es otro trabajo que se podría hacer.

  También hay ideales a derecha que son análogos y tienen una teoría análoga a la de a izquierda.
...
Estas cosas para hacer podrían ser trabajos finales de la Licenciatura en Ciencias de la Computación.
