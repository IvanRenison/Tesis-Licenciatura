...
Siempre que se programa, para que salga bien hay que testear todo, y este trabajo no fue una excepción. A medida que se fue escribiendo el código se fueron haciendo tests del código para eliminar los bugs. Además con algunos de los tests se hizo que hagan mediciones de tiempo para ver que tan rápido andaba el código.
...
A continuación una descripción de los tests  y benchmarks de cada una de las carpetas de la carpeta \texttt{tests}.

\begin{itemize}
  \item \texttt{internal\_tests}: Es en donde están todos los tests que son internos, o sea, no comparan con \texttt{operator\_gb} si no que hacen solo cosas de dentro de la librería. En particular, incluye tests de operaciones básicas de monomios, polinomios, reducciones y ambigüedades, y testeos de que el resultado de llamar a las funciones que guardan la construcción de la respuestas sea el mismo que las que no. En esta carpeta no hay nada de benchmarks.
  \item \texttt{base\_tests}: Es esta carpeta lo que se hace es comprar el resultado de \texttt{./mains/base\_Buchberger.cpp} y \texttt{./mains/base\_F4.cpp}, que toman un conjunto de polinomios y calculan una base de Gröbner del ideal generado por ese conjunto, y se lo compara el resultado con hacer lo mismo usando \texttt{operator\_gb}. La comparación se hace con
\end{itemize}
