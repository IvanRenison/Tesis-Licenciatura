...
Este trabajo presenta una implementación en \cpp para el calculo de las llamadas bases de Grobner no conmutativas. Ya existían previamente algunas implementaciones de dicho cálculo, pero en este trabajo se presenta una que es la primera en \cpp y además incluye un poquito de paralelismo.
...
Para trabajar en algoritmos para abordar el \cref{problem:principal} se considera el conjunto de todos los polinomios no conmutativos que satisfacen ese $∃$, denotado como $(G)$, y se calcula un conjunto de polinomios no conmutativos llamado una base de Gröbner de $(G)$, que puede ser finito o infinito computacionalmente enumerable. Que a veces pase esto último es lo que hace que el problema no sea computable. % Pongo computacionalmente enumerable y no solo enumerable porque enumerable no dice nada
Si se logra obtener una base de Gröbner finita entonces es fácil determinar si $f ∈ (G)$.
...
El capítulo incluye demostraciones de muchos de los resultados expuestos, pero leer las demostraciones no es necesario para entender el trabajo.
...
Como se dijo en la introducción, para esta tesis se implementaron los algoritmos de Buchberger y F4 en \cpp junto con estructuras para manejar polinomios no conmutativos en una librería llamada \texttt{ncgb}. La librería está disponible en el repositorio \url{https://github.com/IvanRenison/Non-commutative-Grobner-Bases}.
...
Como ya se mencionó en el \cref{cap:Benchmarks}, no se sabe para $n ≥ 6$ si los FK$n$ tienen una base finita o no. Cuando se tenga una versión muy buena de F4 , se la podría correr por mucho tiempo para FK6 u otros FK para ver si llega en algún momento a una base finita.
