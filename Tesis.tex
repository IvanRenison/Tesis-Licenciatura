...
\usepackage[backend=biber, autolang=hyphen, bibencoding=inputenc, isbn=false, uniquename=false]{biblatex-chicago}
...
Hay muchas formas de hacerlo en tiempo lineal, la que se usó es usar la función Z (que en el código está en el archivo \texttt{Zfunc.hpp}). En \cite{web:cp-algo:Zfunc} % Citar a cp-algorithms: https://cp-algorithms.com/string/z-function.html
...
\begin{itemize}
  ...
  \item \texttt{ambs} son las ambigüedades que todavía no se procesaron junto con los índices en \texttt{G} de los polinomios a los que corresponde la ambigüedad. Se usa una queue para procesar siempre la que hace mas tiempo está esperando.
