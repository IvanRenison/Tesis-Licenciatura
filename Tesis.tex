...
Mas adelante va a ser necesario tener un orden entre los elementos de $⟨X⟩$, pero no hace falta fijar uno concreto, sino que con que satisfaga la siguiente definición alcanza.
...
\begin{definition}\label{def:reducciones}
  Sean $G ⊆ K⟨X⟩$, $g ∈ K⟨X⟩ - \{0\}$, $a, b ∈ ⟨X⟩$ y $f, f' ∈ K⟨X⟩$. Se define la relacion $→_{≤, G}$ del siguiente modo:
