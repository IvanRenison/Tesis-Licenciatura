...
Un problema de decisión que existe sobre los polinomios no conmutativos es: dado un conjunto $F$ de polinomios no conmutativos y un polinomio no conmutativo $f$ decidir si $f$ se pude escribir como combinación lineal de elementos de $F$ con coeficientes en $K⟨X⟩$, o escrito formalmente determinar si vale la siguiente expresión.
...
Para trabajar en algoritmos para ese problema lo que se hace es considerar el conjunto $(F)$ de todos los polinomios no conmutativos que satisfacen ese $∃$ y se calcula algo que se llama una base de Gröbner de $(F)$, la cual puede ser finita o infinita computacionalmente enumerable. Son estos últimos casos los que hacen que el problema no sea computable.
