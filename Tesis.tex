...
Antes de poder calcular bases de Gröbner algo que vamos a querer hacer es poder computar si un conjunto es una Base de Gröbner, porque ninguna de las equivalencias del \cref{thm:equivalencias de base de Gröbner} es directamente calculable porque hacen cuantificaciones sobre conjuntos infinitos.
...
Se podría pensar que agregar $g$ por ahí cambia el ideal generado, pero por el \cref{lemma:S es cerrado en ideal} sabemos que $g$ es un elemento del ideal, por el \cref{thm:→ mantiene pertenencia a ideal} eso implica que $f$ también pertenece al ideal y por ende por el \cref{lemma:gen G = gen G U a con a ∈ gen G} se puede agregar al conjunto y que el ideal generado siga siendo el mismo.
...
\chapter{Tests y benchmarks}

Siempre que se programa, para que salga bien hay que testear todo, y este trabajo no fue una excepción. A medida que se fue escribiendo el código se fueron haciendo tests del código para eliminar los bugs. Además con algunos de los tests se hizo que hagan mediciones de tiempo para ver que tan rápido andaba el código. Esto último se llama benchmarking. En este capítulo se explican los tests y benchmarks que se hicieron.
...
De los tests hay algunos que corren el código de la librería y el código de \texttt{operator\_gb}, que es la librería hecha por Clemens Hofstalder en su tesis de maestría \cite{thesis:Hof20} usando \texttt{SageMath} y comparan los resultados, y otros que corren solo código de la librería.
...
Cuando se corren los tests, los que tienen además benchmarks imprimen los resultados de los benchmarks. En está sección se presentan esos resultados.
...
\subsection{Conjuntos con bases de Gröbner finitas}

El tests que usa los distintos algoritmos para calcular bases de Gröbner de conjuntos generadores que se sabe que tienen bases de Gröbner finitas mide los tiempos que se demora cada uno y los imprime, dando los siguientes tiempos: % No se bien como decir que lo que estoy poniendo es literalmente el output del test

\begin{minted}{text}
  Test Case                      Buch       | F4         | GB
  ----------------------------------------------------------------------
  Nichols2.in                    0.00s      | 0.01s      | 0.82s
  Nichols3.in                    0.00s      | 0.01s      | 0.81s
  tri1.in                        0.26s      | 0.08s      | 0.98s
  tri10.in                       0.01s      | 0.01s      | 0.86s
  tri11.in                       0.03s      | 0.02s      | 0.90s
  tri12.in                       3.27s      | 0.21s      | 1.04s
  tri13.in                       0.01s      | 0.01s      | 0.85s
  tri2.in                        6.00s      | 0.30s      | 1.19s
  tri3.in                        0.02s      | 0.26s      | 0.92s
  tri4.in                        0.04s      | 0.07s      | 0.96s
  tri5.in                        0.01s      | 0.02s      | 0.89s
  tri6.in                        6.54s      | 1.79s      | 1.30s
  tri7.in                        33.36s     | 1.85s      | 1.48s
  tri8.in                        0.09s      | 0.06s      | 0.94s
  tri9.in                        0.00s      | 0.01s      | 0.84s
\end{minted}

En donde \texttt{Test Case} es el nombre del conjunto generador inicial, \texttt{Buch} es el tiempo con Buchberger, \texttt{F4} es el tiempo con F4 y \texttt{GB} es el tiempo con \texttt{operator\_gb}. A los algoritmos ese es el nombre que le vamos a dar para en resto de la sección, y en las salidas de todos los benchmarks.
...
\subsection{Pertenencia a ideales}
...
El tests que prueba que de lo mismo correr con varios hilos que con uno solo lo que hace es correr F4 para conjuntos que se sabe que tienen una base de Gröbner finita con distintas cantidades de hilos, ver que siempre de igual, e imprime los tiempos. Estos son los resultados:

\begin{minted}{text}
  Test Case   1 threads 2 threads 4 threads 6 threads 8 threads 12 threads 16 threads
  -----------------------------------------------------------------------------------
  Nichols2.in 0.00s     0.00s     0.00s     0.00s     0.00s     0.00s      0.00s
  Nichols3.in 0.00s     0.00s     0.00s     0.00s     0.00s     0.00s      0.00s
  tri1.in     0.09s     0.08s     0.07s     0.07s     0.07s     0.06s      0.06s
  tri10.in    0.01s     0.01s     0.01s     0.01s     0.01s     0.01s      0.01s
  tri11.in    0.02s     0.02s     0.02s     0.01s     0.01s     0.01s      0.01s
  tri12.in    0.32s     0.27s     0.22s     0.22s     0.20s     0.21s      0.20s
  tri13.in    0.00s     0.00s     0.01s     0.00s     0.00s     0.01s      0.01s
  tri2.in     0.54s     0.43s     0.35s     0.32s     0.31s     0.30s      0.29s
  tri3.in     0.03s     0.03s     0.03s     0.03s     0.03s     0.03s      0.03s
  tri4.in     0.07s     0.06s     0.06s     0.06s     0.06s     0.06s      0.06s
  tri5.in     0.01s     0.01s     0.01s     0.01s     0.01s     0.01s      0.01s
  tri6.in     2.22s     2.02s     1.84s     1.85s     1.87s     1.76s      1.81s
  tri7.in     2.22s     2.07s     1.90s     1.85s     1.85s     1.83s      1.83s
  tri8.in     0.07s     0.06s     0.06s     0.05s     0.05s     0.05s      0.05s
  tri9.in     0.00s     0.00s     0.00s     0.00s     0.00s     0.00s      0.00s
\end{minted}
% Esta tabla no está tal cual el output si no que está achicada para que entre, ¿que debería hacer?
