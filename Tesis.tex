...
\def\tituloTesis{Implementación de bases de Gröbner no conmutativas en C++ con un poquito de paralelismo}
\input{Carátula}
...
Algo a destacar de la implementación que se hizo es que funciona para cualquier cuerpo. De las implementaciones ya existentes, según conocimiento del autor, ninguna funciona para cualquier cuerpo.
...
\begin{definition}
  ...
  con las siguientes operaciones: la suma de los elementos de $R⟨X⟩$ como unir las $∑$, el producto por escalares de definido como
  \[ c (∑_{i = 1}^n c_i m_i) = (∑_{i = 1}^n c c_i m_i) \text{,}\]
  y el producto entre elementos de $R⟨X⟩$ como
...
Que $→_{≤, G}$ fuera confluente siempre sería muy útil porque significaría que para cualquier clase de equivalencia de $≡_{(G)}$ podríamos siempre llegar a una misma forma normal y así poder determinar si dos elementos son equivalentes. En particular poder determinar si un elemento está en el ideal viendo si se llega a $0$ como la forma normal. Los casos en los que sí es confluente se llaman bases de Gröbner y después algo que vamos a hacer es calcular una base de Gröbner de un ideal generado por un conjunto que no es base de Gröbner.
...
Estas definiciones son un diccionario polinomios-matrices y matrices-polinomios. Así, lo que antes en la reducción era restar un polinomio multiplicado por un escalar y un monomio a cada lado, la idea es que ahora pase a ser restarle una fila multiplicada por un escalar a otra en la matriz. Para eso, al comenzar hay que tener en la matriz los polinomios a restar ya multiplicados por los monomios a cada lado, es decir, con los $agb$ con $a,b ∈ ⟨X⟩$ y $g ∈ G$. Para eso definimos el algoritmo de preprocesamiento simbólico que, dado el conjunto por el cual se reduce $G$ y el conjunto a reducir $F$, calcula todos los polinomios que van a ser necesarios:
...
Como se dijo al principio, para esta tesis se implementaron los algoritmos de Buchberger y F4 en C++ junto con estructuras para manejar polinomios no conmutativos en una librería llamada \texttt{ncgb}. En este capítulo se explica cómo usar esa librería en su versión actual (fecha de publicación de esta tesis). Esa librería está en GitHub en el repositorio \href{https://github.com/IvanRenison/Non-commutative-Grobner-Bases}{\texttt{Non-commutative-Grobner-Bases}}.
...
La primera es la carpeta \texttt{ncgb}, que es en donde está alojada la librería en sí. Tiene muchos archivos \texttt{.hpp} que son los que hay que incluir para usar la librería y que tienen la implementación ahí mismo. En algunas librerías a veces hay archivos \texttt{.h} con las declaraciones y archivos \texttt{.cpp} con las implementaciones, pero acá eso no es así por el uso de templates.
...
Los polinomios como los definimos en el \cref{cap:Preliminares} están asociados a un cuerpo y un conjunto de variables, y además muchas cosas de los polinomios (y de otras definiciones) dependen de un orden monomial. Para que al usar el código también se puedan variar esas tres cosas todas las funciones y estructuras están hechas como templates con esas tres cosas como argumentos. Por ejemplo, la definición de polinomio es así:
...
\begin{itemize}
 \item \texttt{+}, \texttt{-} (tanto unario como binario), \texttt{*} y \texttt{/} junto con sus versiones de asignación \texttt{+=}, \texttt{-=}, \texttt{*=} y \texttt{/=}. De las versiones asignación no se usa el resultado, solo el que modifican el operando izquierdo, así que pueden devolver \texttt{void}.
 ...
 \item Un constructor que ande con con \texttt{0} y con \texttt{1}, o sea, que se pueda hacer \texttt{K(0)} y \texttt{K(1)}.
\end{itemize}
...
En el código todos los nombres y comentarios están en inglés, así que acá todos los comentarios que vienen del código están también en inglés.
...
El código de este ejemplo lo que hace es, trabajando sobre los racionales, leer un conjunto generador en formato numérico, calcularle una base de Gröbner asumiendo que tiene una base finita e imprimir la base de forma bonita.
...
Una librería que hace la reducción por filas es FLINT, que lo hace para el tipo \texttt{mpq\_class} de GMP que ya dijimos que son los números racionales \cite{lib:flint, lib:gmp}. Para usarla, en el archivo \texttt{matrix\_mpq\_class.hpp} hay una especialización de \texttt{rref} para \texttt{mpq\_class}.

¿Qué es una especialización en C++? Recordatorio para quienes no lo recuerdan bien: Es cuando hay código definido con un template y se hace una definición aparte para alguna combinación particular de parámetros del template.
...
Esta optimización no se implementó principalmente por el poco soporte para trabajar con matrices de un cuerpo arbitrario, o de una implementación específica de $ℚ$, que hay en C++. Sin embargo, queda como trabajo futuro en la \cref{section:trabajos futuros}.
...
La función \texttt{cmpBases} que se mencionó en la \cref{section:Comparación de bases de Gröbner (libreria)} que toma dos conjuntos generadores y asumiendo que son bases de Gröbner dice si generan el mismo ideal o no está implementada usando el siguiente teorema.
...
Con este teorema la función \texttt{cmpBases} es una implementación tal cual de este teorema.
...
Algunos tests corren el código de la librería y el código de \texttt{operator\_gb}, que es la librería hecha por Clemens Hofstadler en su tesis de maestría \cite{thesis:Hof20} usando SageMath, y comparan los resultados, y otros corren solo código de la librería.
...
Algunos de los tests al mismo tiempo que testean miden los tiempos para tener benchmarks.
...
De cualquier manera esos datos no son muy importantes porque lo que importa son las relaciones entre los distintos tiempos.

Los tests que tienen benchmarks son todos salvo uno tests que usan los mains de la carpeta \texttt{mains} y mains hechos en Python con \texttt{operator\_gb} y son corridos con distintas entradas desde un archivo de Python. Los tiempos se miden desde el programa de Python, así que incluyen el tiempo hasta que se inicializa el programa y lee la entrada y el tiempo de impresión de la salida, pero como son siempre entradas y salidas relativamente chicas eso no debería ser problema.
...
El test que usa los distintos algoritmos para calcular bases de Gröbner de ideales que se sabe que tienen bases de Gröbner finitas incluye mediciones de los tiempos y por supuesto usa los casos que vimos recién.

En el siguiente gráfico se muestran los tiempos de ejecución de Buchberger, de F4 y de \texttt{operator\_gb} para esos casos.
...
A simple vista se puede ver que para los casos en los que se distingué algo \texttt{operator\_gb} es mucho más rápido, pero solo se ven los casos que demoran mucho tiempo los otros no. Así que a continuación está el gráfico de vuelta pero solo con la parte de mas abajo y dejando que las barras muy largas se salgan del gráfico.
...
De vuelta a simple vista no se ve casi nada, salvo que en trit5 es caso lo mismo con mas hilos. Para porder ver en mas detalla, a continuación un gráfico de lo mismo pero sin trit5.
...
El test que compara ejecutar Buchberger con y sin construir la respuesta también usa los mismos casos y mide los tiempos. Este es el único benchmark que no funciona con mains de la carpeta \texttt{mains} si no que usa sus propios archivo, y además este benchmark mide los tiempos desde C++. En el siguiente gráfico se pueden ver los resultados.
...
\noindent \begin{tikzpicture}
  \begin{axis}[
      ...
      enlarge x limits = 0.03, % Espaciado en los bordes
      legend style = {at = {(0.024, 0.94)}, anchor = north west}, % Estilo de la leyenda
      legend entries = {Normal, Construcción de la respuesta}, % Entradas de la leyenda
  ]
    % Datos para cada conjunto
...
Y estos para los enteros módulo un primo (también tal cual los imprime el test):
...
Estos datos igualmente mucho no dicen, porque como buscan por una cierta cantidad de pasos, y los pasos son cosas distintas en los distintos algoritmos, no se puede comparar. En particular, un paso de F4 puede corresponder a muchos pasos de Buch.
...
\section{Como fue el proceso de hacer este trabajo}\label{section:proceso}

El tema de este trabajo fue propuesto por el director Cristian Vay por su interés ya mencionado de usarlo para las álgebras de Nichols. Para el autor este era un tema completamente nuevo, así que empezó por estudiar el tema. Primero leyendo \cite{book:ideals-varieties-algorithms} que trata sobre bases de Gröbner si conmutativas, mientras leía ese libro hizo también una implementación así nomás de bases de Gröbner si conmutativas en C++. Esto fue bueno porque sirvió de práctica para después hacer la implementación de bases de Gröbner no conmutativas. Después de eso el autor fue leyendo \cite{thesis:Hof20} y \cite{phdthesis:Hof23}, que son sobre bases de Gröbner no conmutativas, e implementando lo explicado.

\section{Cosas buenas y cosas malas de este trabajo}

Algunas cosas buenas de este trabajo:

\begin{itemize}
  \item Es la primer implementación de bases de Gröbner no conmutativas en C++ (que el autor sepa). Esto es muy bueno porque C++ es un lenguaje particularmente rápido y en el que es fácil paralelizar. Esto es útil si se quieren correr casos que se demoran mucho.
  \item Es la primer implementación de F4 que funciona para un cuerpo arbitrario (que el autor sepa). Esto está bueno para que si en algún momento alguien lo quiere usar para algún cuerpo distinto lo pueda hacer.
  \item La implementación de Buchberger incluye la parte de la construcción de la respuesta, que si bien es algo muy fácil, la única otra implementación que lo hace es \texttt{operator\_gb} (que el autor sepa).
  \item El \cref{cap:Preliminares} de esta tesis es la primer explicación en español de bases de Gröbner no conmutativas (que el autor sepa).
\end{itemize}

Algunas cosas malas de este trabajo:

\begin{itemize}
  \item No está hecha la optimización de usar la reducción de Faugère-Lachartre. Esto es sin duda el mayor punto negativo del trabajo.
  \item Que sea en C++ también tiene como parte negativa que, como el manejo de librerías en C++ es bastante desorganizado, si alguien quiere usar la librería tiene que arreglárselas para hacerla andar, porque no está todo solucionado para poder instalarla y usarla fácilmente. Algunas librerías de C++ tienen algo para que se puedan instalar de forma relativamente sencilla, pero eso acá no está hecho porque no parece fácil de hacer (el autor nunca hizo nada parecido). De cualquier manera, \texttt{operator\_gb} como usa SageMath tampoco es fácil de instalar, porque SageMath no es una librería de Python común que se instale con PIP si no que hay que hacer cosas más complicadas para instalarlo.
\end{itemize}

\section{Trabajos futuros}\label{section:trabajos futuros}

A continuación una lista trabajos que se podrían hacer sobre el tema.

\begin{itemize}
  \item Como ya se dijo varias veces, falta usar la reducción por filas de Faugère-Lachartre que es más eficiente, y esa es una de las cosas más importantes para hacer.

  \item Del algoritmo F4 (y de Buchberger) para el caso si conmutativo hay varias implementaciones en C++, por ejemplo \cite{lib:openf4, lib:mathic, lib:M4GB}, inclusive en paralelo, como \cite{DBLP:journals/jsc/Reeves98, lib:parallelGBC}. Muchas son solo para los enteros módulo un primo, pero hay algunas que también para los racionales. Es muy probable que analizando esas implementaciones se puedan sacar muchas cosas útiles, en particular, podría ser que ya alguien haya hecho la implementación de la reducción por filas de Faugère-Lachartre, ya que posiblemente sirva también para el caso no conmutativo.

  \item Cuando se fue haciendo la librería, muchas veces pasaba que había cambios que parecían optimizaciones pero al hacerlos el código resultaba andar mas lento, así que se descartaban esos cambios. Sería bueno agregar mas test y probar hacer cambios para ver si mejoran o no los tiempos teniendo más tests.

  \item Se podría analizar cuanto tiempo se consume en las distintas partes de los algoritmos, porque eso podría ayudar a saber en que partes se pueden realizar a optimizaciones.

  \item Además de los ideales de la \cref{def:ideal} existen los ideales a izquierda que se definen de la siguiente manera.
  ...
  Con los ideales a izquierda sobre polinomios no conmutativos también hay toda una teoría de bases de Gröbner. En \cite{phdthesis:Hof23} se puede encontrar una explicación sobre esta teoría. Implementar el cálculo de bases de Gröbner de la teoría de ideales a izquierda es otro trabajo que se podría hacer. Análogamente, también se pueden considerar ideales a derecha.

  \item Como ya se mencionó en el \cref{cap:Benchmarks}, los FK$n$ no se sabe para $n ≥ 6$ si tienen una base finita o no. Cuando se tenga una versión muy buena de F4 algo que se podría hacer es correrla por mucho tiempo para FK$n$ para ver si llega en algún momento a una base finita.

  \item Actualmente para estudiar el tema de bases de Gröbner, tanto si conmutativas cono no conmutativas, hay libros y artículos sobre el tema, en particular \cite{book:ideals-varieties-algorithms} para si conmutativas y \cite{thesis:Hof20} para no conmutativas, pero para practicar las implementaciones no hay mucha ayuda. Por ejemplo, cuando el autor implementó las bases de Gröbner si conmutativas para practicar tubo que hacer su propio testing y todo. Para que estudiar el tema sea más fácil algo que sería útil es preparar del problemas estilo de programación competitiva sobre el tema. Estos problemas se podrían poner en, por ejemplo, \href{https://judge.yosupo.jp/}{Library Checker}, o en un grupo de \href{https://codeforces.com/}{Codeforces}.

  \item Por último, otra cosa que se podría hacer es implementar el calculo de bases de Gröbner no conmutativas en otros lenguajes como por ejemplo Rust.
\end{itemize}
